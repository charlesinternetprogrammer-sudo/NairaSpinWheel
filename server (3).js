...
function handleWebhookEvent(event, isFromApiWebhook = false) {
  ...
  const endpoint = isFromApiWebhook ? '[/api/webhook]' : '[/webhook]';
  console.log(`\n${endpoint} ── ${appName} ── Event: ${type} | Ref: ${ref}`);
  console.log(`${endpoint} Data:`, JSON.stringify(d, null, 2));
  ...
  return { status: 'received', app: appName, ref };
}
...
app.get('/', (req, res) => {
  res.json({
    success     : true,
    service     : 'IncomePM — Unified Backend Server',
    version     : '1.0.0',
    status      : 'running',
    ready       : !!FLW_SECRET_KEY,
    timestamp   : nowISO(),
    earn_rate   : `₦${EARN_RATE} per minute`,
    apps_handled: ['IncomePM (IPM-)', 'NairaSpinWheel (NSW-)'],
    routes: {
      'GET  /'                        : 'API directory — this page',
      ...
      'POST /api/webhook'             : '⚡ Unified API webhook — Transfers & Deposits (DISCOVERABLE)',
      'GET  /api/webhook'             : '✅ Webhook endpoint health check',
    },
    webhook_info: {
      primary_endpoint   : `${req.protocol}://${req.get('host')}/webhook`,
      api_endpoint       : `${req.protocol}://${req.get('host')}/api/webhook`,
      description        : 'Both endpoints are identical unified webhooks. Use either for Flutterwave callbacks.',
      event_types        : ['transfer.completed (withdrawals)', 'charge.completed (deposits)'],
      routing_logic      : 'Routes by reference prefix: IPM-* = IncomePM, NSW-* or SPT-* = NairaSpinWheel',
    },
    test_urls: {
...
// Verify webhook endpoint is active and ready to receive events
// ─────────────────────────────────────────────────────────────────
app.get('/api/webhook', (req, res) => {
  res.json({
    success: true,
    message: 'Webhook endpoint is active and ready to receive Flutterwave events.',
    endpoint: 'POST /api/webhook',
    methods: ['POST'],
    description: 'Send POST requests with Flutterwave transfer/charge events to this endpoint.',
    event_types: ['transfer.completed', 'charge.completed'],
    ready: !!FLW_WEBHOOK_HASH,
    app_types: ['IncomePM (IPM-*)', 'NairaSpinWheel (NSW-* or SPT-*)'],
    timestamp: nowISO(),
  });
});
...
app.post('/webhook', express.raw({ type: 'application/json' }), (req, res) => {
  ...
  const result = handleWebhookEvent(event, false);
  res.status(200).json(result);
});

// ─────────────────────────────────────────────────────────────────
// POST /api/webhook
// ─── UNIFIED API WEBHOOK — handles IncomePM + NairaSpinWheel ─────
// Identical to /webhook. Both endpoints are discoverable.
// Routes by reference prefix: IPM- = IncomePM, NSW- = NairaSpinWheel
// ─────────────────────────────────────────────────────────────────
app.post('/api/webhook', express.raw({ type: 'application/json' }), (req, res) => {
  const signature = req.headers['verif-hash'];
  if (FLW_WEBHOOK_HASH && signature !== FLW_WEBHOOK_HASH) {
    console.warn('[/api/webhook] Invalid signature — rejected');
    return res.status(401).json({ message: 'Invalid signature' });
  }
  ...
  const result = handleWebhookEvent(event, true);
  res.status(200).json(result);
});
...
app.listen(PORT, () => {
  ...
  console.log(`   • POST /api/webhook (DISCOVERABLE)`);
  console.log(`   • GET  /api/webhook (HEALTH CHECK)`);
  console.log(`   Both POST routes handle transfers & deposits for IncomePM + NairaSpinWheel\n`);
});
